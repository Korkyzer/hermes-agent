...
class TestSchemaInit:
    ...
    def test_search_cache_expired_row_is_ignored(self, db):
        db.set_search_query_cache("hash1", "deploy", '{"ok": true}', ttl=-1)
        assert db.get_search_query_cache("hash1") is None
...
class TestFTS5ToolCallMigration:
    ...
    def test_v11_to_v12_replaces_message_triggers_for_session_search_fts(self, tmp_path):
        """Existing v11 DBs must index messages inserted after v12 migration."""
        import sqlite3
        ...
        session_db = SessionDB(db_path=db_path)
        try:
            session_db.create_session("new_session", "cli")
            session_db.set_session_title("new_session", "Fresh migration title")
            session_db.append_message(
                "new_session",
                role="user",
                content="fresh migration body token",
            )

            assert len(session_db.search_messages("fresh")) == 1
            assert len(session_db.search_messages("Fresh")) == 1

            trigger_sql = session_db._conn.execute(
                "SELECT sql FROM sqlite_master "
                "WHERE type = 'trigger' AND name = 'messages_fts_insert'"
            ).fetchone()[0]
            assert "session_search_fts" in trigger_sql
        finally:
            session_db.close()
