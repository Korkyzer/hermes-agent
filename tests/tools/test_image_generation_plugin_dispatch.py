...
class TestPluginDispatch:
    ...
    def test_dispatch_rejects_references_for_unsupported_provider(self, monkeypatch, tmp_path):
        from tools import image_generation_tool
        from hermes_cli import plugins as plugins_module
        from agent import image_gen_registry as registry_module
    ...
    def test_dispatch_forwards_references_to_supported_provider(self, monkeypatch, tmp_path):
        from tools import image_generation_tool
        from hermes_cli import plugins as plugins_module
        from agent import image_gen_registry as registry_module

        class _ReferenceProvider(_FakeCodexProvider):
            seen = None
            ...
            def generate(self, prompt, aspect_ratio="landscape", **kwargs):
                self.seen = kwargs.get("references")
                result = super().generate(prompt, aspect_ratio, **kwargs)
                result["references"] = self.seen
                return result
        ...
        assert payload["success"] is True
        assert payload["references"] == ["/tmp/ref.png"]

    def test_materialize_downloads_reference_url_to_cache_without_preserving_query(self, monkeypatch, tmp_path):
        from tools import image_generation_tool
        ...
        assert len(refs) == 1
        assert refs[0].startswith(str(tmp_path / "cache" / "image_references"))
